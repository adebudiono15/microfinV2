                                     ...
                                     {{--  Pengeluaran Langsung  --}}
                                        <div class="table-responsive">
                                            <table id="basic-datatables" class="display table table-striped table-hover" >
                                                ...
                                                <tbody>
                                                    @foreach ($pengeluaranlangsung as $e=>$item)
                                                    <tr>
                                                        <td>{{ $e+1 }}</td>
                                                        <td>{{ Str::limit($item->kode_pengeluaran_tunai,10) }}</td>
                                                        <td>{{ Str::limit($item->supplier->nama,10) }}</td>
                                                        <td>{{ number_format($item->jumlah_pengeluaran,0) }}</td>
                                                        <td class="text-center" style="width: 300px">
                                                            <a href="#" data-id="{{ $item->id }}"
                                                                class="btn btn-sm btn-dark btn-shadow mr-2 mt-2 mb-2 btn-detail-pengeluaran-langsung">
                                        ...
                                        <div class="table-responsive">
                                            <table id="basic-datatables-kredit" class="display table table-striped table-hover" >
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        <td>{{ $e+1 }}</td>
                                                        <td>{{ Str::limit($item->kode_pengeuaran_bank,10) }}</td>
                                                        <td>{{ Str::limit($item->supplier->nama,10) }}</td>
                                                        <td>{{ number_format($item->jumlah_pengeluaran,0) }}</td>
                                                        <td class="text-center" style="width: 300px">
                                                            <a href="#" data-id="{{ $item->id }}"
...
<script>
        ...
        $('.btn-detail-pengeluaran-langsung').on('click', function() {
            let id = $(this).data('id')
            $.ajax({
                url: `/${id}/detail-pengeluaran-langsung`,
                method: "GET",
